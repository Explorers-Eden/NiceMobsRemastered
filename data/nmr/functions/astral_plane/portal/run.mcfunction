execute as @e[type=player] at @s unless entity @e[type=marker,tag=nmr_astral_plane_exit,distance=..16] run return fail

execute as @e[type=marker,tag=nmr_astral_plane_exit] at @s if entity @e[type=player,distance=..2] run function nmr:astral_plane/portal/get_tp_pos

$run kill @e[type=text_display,tag=nmr_level_display,tag=$(uuid_0)$(uuid_1)$(uuid_2)$(uuid_3)]
data modify entity @s DeathLootTable set value "nmr:discs"
...
$summon text_display ~ ~ ~ {billboard:"vertical",shadow:1b,Tags:["nmr_rarity_display","$(uuid_0)$(uuid_1)$(uuid_2)$(uuid_3)"],transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0.3f,0f],scale:[.5f,.5f,.5f]},text:'{"text":"⚔ $(first_name) $(last_name)","color":"#58CD78","bold":false,"italic":false}',background:16711680}
